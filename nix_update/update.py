...
def update_version(package: Package, version: str, version_extractor: str) -> bool:
    if version == "auto":
        if not package.url:
            if package.urls:
                url = package.urls[0]
        ...
        new_version = fetch_latest_version(url, version_extractor)
    else:
        new_version = version
...
def update(opts: Options) -> Package:
    ...
    if opts.version != "skip":
        update_hash = update_version(package, opts.version, opts.version_extractor)
