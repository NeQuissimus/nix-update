...
def fetch_gitlab_version(url: ParseResult, version_extractor: str) -> Optional[str]:
    ...
    for tag in tags:
        if tag["release"]:
            ...
            extracted = extract_version(name, version_extractor)
            if extracted is not None:
                return extracted
    # if no release is found, use latest tag
    name = tags[0]["name"]
    assert isinstance(name, str)
    return extract_version(name, version_extractor)
